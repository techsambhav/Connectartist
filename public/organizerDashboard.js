...
(async function () {
  ...
  // Client-side guard: ensure current session role is organizer; otherwise, show guidance.
  try {
    const roleRes = await fetch('/api/auth/session-role', { credentials: 'include' });
    if (roleRes.status === 401) {
      container.innerHTML = '<div class="empty">Please sign in as Organizer to view this page. <a href="/api/auth/google/organizer">Login as Organizer</a></div>';
      return;
    }
    const roleJson = await roleRes.json().catch(() => ({}));
    const sessionRole = roleJson && roleJson.sessionRole;
    if (sessionRole !== 'organizer') {
      container.innerHTML = '<div class="empty">You\'re logged in as an artist. Please <a href="/api/auth/logout?redirect=/organizerlogin.html">log out</a> and then <a href="/api/auth/google/organizer">Login as Organizer</a>.</div>';
      return;
    }
  } catch (e) {
    // if check fails, continue and let API enforce
  }
